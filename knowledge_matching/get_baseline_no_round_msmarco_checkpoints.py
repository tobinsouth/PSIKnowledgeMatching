"""
Script Name: get_baseline_no_round_msmarco_checkpoints.py

Summary:
    This script processes the MSMARCO dataset to compute baseline embeddings and retrieval metrics
    without rounding. It saves corpus embeddings at checkpoints and computes evaluation metrics,
    which are saved to a CSV file. The script assumes that the following files from a previous run
    already exist:
    - datasets/{DATASET}/query_embeddings.pt
    - datasets/{DATASET}/query_ids.json
    - datasets/{DATASET}/corpus_ids.json
    - datasets/{DATASET}/qrels_full.json

    The script is designed to handle scenarios where the server crashes during corpus encoding,
    providing the user with the capability to rerun it and resume from the last checkpoint prior
    to the crash. This feature is particularly valuable given the extensive time required for corpus encoding.

Usage:
    python3 get_baseline_no_round_msmarco_checkpoints.py
